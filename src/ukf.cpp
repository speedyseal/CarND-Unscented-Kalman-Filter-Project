  ...
  */
  ...
  for (int i = 0; i< 2*n_aug_+1; i++)
  {
    ...
    if (fabs(yawd) > 0.001) {
      ...
      cout << "Warning: avoid prediction division by zero for yawd." << endl;
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  // create matrix for incoming measurement
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    if (den < 0.001)
    {
      cout << "regularizing radr meas denominator" << endl;
      den = 0.001;
    }
    Zsig(2,i) = (p_x*v1 + p_y*v2 ) / den;   //r_dot
  }
